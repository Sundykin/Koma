...
当前图是纯 DAG，无法表达：
- 条件执行（如"如果图片质量 < 阈值则重新生成"）
- 循环（如"迭代优化 3 次"）

引入控制流节点：
- `linghui/switch` → 根据输入条件选择分支
- `linghui/loop` → 执行子图 N 次或直到条件满足
- `linghui/gate` → 人工审核节点（暂停执行等待确认）

#### 5.3.2 实时协同编辑

基于 CRDT（如 Yjs）实现多人同时编辑同一灵绘画布：
- 节点位置和属性通过 CRDT 同步
- 执行状态通过事件广播
- 冲突解决策略：最后写入胜出（节点属性），合并（新增节点/边）
