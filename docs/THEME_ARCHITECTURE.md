...
| SCSS `$lh-*` | `linghui/page/styles/_tokens.scss` | **完全独立硬编码** |
| 散落 hex / rgba | 各 `*.css` / `*.tsx` | 889 行 hex 命中 |

**5 份"事实"互不引用** → 任何主题改动都要在 5 处手动同步 → 永远不一致。
...
灵绘 `_tokens.scss` 改造同此（SCSS 变量做语义别名，值来自 CSS 变量）：

```scss
// linghui/page/styles/_tokens.scss
$lh-bg-page:           var(--token-bg-app);
$lh-bg-overlay-soft:   var(--token-overlay-on-bg);
$lh-text-primary:      var(--token-text-primary);
$lh-accent-primary:    var(--token-accent-base);
$lh-shadow-panel:      var(--token-shadow-md);
```
...
themeProvider.setTheme('light-business')
        ...
        │       (Tailwind 工具类、SCSS module、灵绘 SCSS 全部跟随)
        │
...
| 4 套主题切换 5 关键页（分镜/灵绘/设置/编辑器/资产）截图无错位 | ✓ |
