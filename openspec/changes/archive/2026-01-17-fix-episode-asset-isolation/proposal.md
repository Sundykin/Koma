...
修复分集资产隔离问题，重构角色生成流程，支持远程 URL 固化以及 Sora2 角色绑定。

## Problem

1. **分集资产未隔离**：切换到第二集时，第一集的所有资产仍然显示，没有按分集筛选
2. **缺少 AI 资产复用分析**：新分集分析时没有调用 AI 识别可复用的已有资产
...
- AssetManager 根据当前分集的 `characterRefs/sceneRefs/propRefs` 筛选显示资产
- 提供"显示全部项目资产"开关，默认只显示当前分集关联的资产

### 2. AI 资产复用分析

- 分集分析时，LLM 提取的角色/场景/道具与项目已有资产进行匹配
- 高置信度自动复用，中置信度提示用户确认
...
1. 切换分集时，AssetManager 只显示当前分集关联的资产
