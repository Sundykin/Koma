...
interface WriteQueueItem {
  key: string;
  execute: () => Promise<any>;
  waiters: Array<{ resolve: (value: any) => void; reject: (reason?: any) => void }>;
...
export class PersistenceService {
  ...
  private enqueueWrite<T>(key: string, execute: () => Promise<T>): Promise<T> {
    ...
    if (existing) {
      // Merge: update the execute function and add a new waiter
      ...
      existing.timer = setTimeout(async () => {
        ...
        try {
          const result = await existing.execute();
          for (const w of existing.waiters) w.resolve(result);
        } catch (error) {
          for (const w of existing.waiters) w.reject(error);
        }
      }, 50);
