  ...
  async query(request: LLMQueryRequest): Promise<LLMQueryResponse> {
    ...
    while (true) {
      ...
      try {
        ...
        // responseFormat: 仅原生 OpenAI API 支持 JSON 强制模式
        // OpenAI 兼容接口（自定义 baseUrl）不一定支持 response_format，盲目启用可能导致请求卡住或超时
        const wantsJsonObject = request.options?.responseFormat === 'json_object';
        const isNativeOpenAI = sessionConfig.modelProvider === 'openai' &&
          (!sessionConfig.baseUrl || /\bopenai\.com\b/i.test(sessionConfig.baseUrl));
