...
4. **台词用引号自然融入**：\`角色名："台词内容"\`，说话人写在引号前（如\`叶赎抬眼："你们来了。"\`），一段可有多句对话。旁白/画外音不是必须的，有才写，自然写成"画外音传来：……"。
...
`,
  ...
  shot_script_rewrite: {
    id: 'shot_script_rewrite',
    category: 'analysis',
    name: '分镜脚本换拍法',
    description: '保持剧情/台词/动作，重写画面表达（换景别/机位/光线组合），探索不同画面感',
    template: `你是专业的影视分镜师。下面是分镜脚本。**换一种拍法改写它**：保持剧情事件、人物动作、台词（引号原样一字不改），但换一种画面表达——可尝试不同的景别/机位/光线组合，让画面更有张力、更贴合剧情氛围。**只输出改写后的脚本正文，不要解释、不要 JSON**。

【改写要求】
1. **保留**：剧情事件、人物动作、台词（引号原样一字不改）。
2. **换拍法**：尝试与原脚本不同的景别（特写/近景/中景/全景/大全景）或机位（平视/俯视/仰视/低机位）组合，搭配相应的光线与构图，写出与原来不同的画面感。
3. **输出完整一段**：景别开头 + 机位 + 光线 + 构图 + 引号台词，自然行文（不要列清单）。
4. **严禁**：设定堆砌 / 外貌清单复述 / 心理描写 / 评价句 / 概括性叙述——只写当下能被摄影机拍到的内容。
...
{{script}}`,
  ...
  shot_script_upgrade: {
    id: 'shot_script_upgrade',
    category: 'analysis',
    name: '分镜脚本摄影语言补全',
    description: '把缺摄影语言的分镜脚本升级为专业剧本描述：保留剧情/台词/动作，补全景别/机位/光线/构图',
    template: `你是专业的影视分镜师。下面是一个分镜脚本，可能缺少摄影语言（没有景别/机位/光线）。把它改写为专业版本，**只输出改写后的脚本正文，不要解释、不要 JSON**。

【改写要求】
1. **保留**：剧情事件、人物动作、台词（引号原样保留）；不改变任何剧情事实。
2. **补全摄影语言**并自然融入行文（不要列清单）：
   - 景别：行文开头点明——特写 / 近景 / 中景 / 全景 / 大全景
   - 机位视角：平视 / 俯视 / 仰视 / 低机位
   - 光线：光源方向 + 色温 + 明暗对比（如"侧上冷白余光映亮半张脸，其余陷于暗部"）
   - 构图：主体在画面中的位置、前景/背景层次
3. **台词用引号**：\`角色名："台词内容"\`，说话人写在引号前，保留原台词一字不改。
4. **严禁**：设定堆砌 / 外貌清单复述 / 心理描写 / 评价句 / 概括性叙述（"两人激战"）——只写当下能被摄影机拍到的内容。
