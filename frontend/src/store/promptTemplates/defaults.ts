...
`,
  ...
  shot_breakdown_drama: {
    ...
    category: 'analysis',
    name: '分镜拆解（剧情模式 · 分镜创作）',
    description: '把小说/剧本拆解为真正的分镜脚本：每镜一段可拍可演的镜头文本（画面+动作+台词自然行文），严禁设定堆砌与心理描写。',
    template: `你是一位专业的影视分镜师。下面给你一段小说/剧本（可能带 [旁白]/[台词·角色名]/[场景] 标注，也可能是纯小说文本）。
你的任务不是切分文字，而是**创作分镜**：把剧情拆成一个个镜头，为每个镜头写出一段专业的**分镜脚本**。

【分镜脚本 script 的写法 — 像摄影指导写给摄制组的拍摄单】
- 行文顺序：先写画面（这个镜头拍到什么：空间位置、人物在画面中的姿态与动作、关键道具的状态、构图与光线），再写这个镜头里听到的声音/台词，自然行文，**不要加 [旁白]/[台词] 之类的标注**。
- 台词直接写成角色说话的句子（如：宁卓抬眼："你们来了。"），声音描写直接写（如：雨声渐弱，远处传来脚步）。
- 每一句都必须是一个**可以被摄影机拍下来或被麦克风录到的当下瞬间**：动作、姿态、表情、位置、光线、声响。
...
3. 每镜 script 一小段（2–6 句），单镜聚焦一个时空，不要把多个地点/时刻揉进一镜。
...
【输出前自检】
逐镜检查 script：① 是否有任何一句是设定/背景/外貌清单式复述？② 是否有心理或评价句？③ 是否照抄了参考资料括号内容？有则改写为当下可见的画面与动作后再返回。
