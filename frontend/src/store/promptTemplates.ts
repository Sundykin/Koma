...
请严格按照下面的爆款公式 + 漫剧文案规则，输出一段可直接做 TTS 配音的连续旁白。
...
- 每句单句 8–22 字，按 1.3–1.5 倍语速朗读约 2–4 秒/句（这是**单句节奏**要求，不是总字数要求）
- 关键转折前用「……」或短句制造停顿
...
【10. 输出前自检（必须在交付前自我核对）】
═════════════════════════════════════════════════════════════
内部完成草稿后，**逐条**核对下面 6 条；任意一条不达标，自我修订后再输出最终版本：
1. ☐ 全文统一第一人称"我"，没有切到第三人称解说？
2. ☐ 开头第 1 句是上述 9 种爆款公式之一的钩子，不是平铺直叙？
3. ☐ 原剧本里每一个**核心剧情节点**（设定 / 起因 / 冲突 / 转折 / 高潮 / 结局留扣）都有旁白覆盖，没有跳节？
4. ☐ 原剧本里每个**关键画面动作 / 关键台词 / 关键道具 / 空间转换**都有对应旁白，没有为了短节奏而跳过画面？
5. ☐ 单句长度在 8–22 字区间，节奏紧凑（**不在乎总字数**——内容齐全优先于篇幅）？
6. ☐ 没有"接下来 / 画面切到 / 我们看到"等元解说语言；没有 Markdown 标题 / 标签 / 代码块？

═════════════════════════════════════════════════════════════
【输出格式】
═════════════════════════════════════════════════════════════
直接输出连续旁白文本，每句一行（用换行分隔）。
...
3. 每个分镜分配 1–3 句旁白；每句仍保持 8–22 字、口语化、有画面感。
...
\`\`\`json
[
  { "shotIndex": 1, "tweetCopy": "我一个重度精神病，却被全国人奉为神明。" },
  { "shotIndex": 2, "tweetCopy": "他们听我说自己是派大星，竟也深信不疑……" }
]
\`\`\`

字段定义：
- \`shotIndex\`：分镜清单里的编号（从 1 开始），与输入顺序严格对应
- \`tweetCopy\`：分给该分镜的旁白文本（第一人称"我"），多句之间用一个空格连接成一行
`,
