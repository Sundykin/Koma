...
/**
 * 决定 shots 段渲染模式。优先级：
 *   1. explicitCellCount（**用户意图**，来自 shot.imageMode）—— 即使锚定图还没生成，
 *      只要用户选了 grid-4 / grid-9，就按对应模式渲染；
 *   2. bundle.gridCellCount（运行时锚定图存在时的派生值）；
 *   3. 默认 normal（2-3 镜头硬切）。
 *
 * 之前的 bug：当用户切到 grid-4 但还没生成 3×3 / 2×2 图时，bundle.hasGridAnchor=false
 * 导致整条链路退化到 normal，shotsSection 输出 2-3 镜头骨架，跟用户期望不符。
 */
...
function renderNormalShotsSection(duration: number): string {
  ...
  const shotTemplate = (idx: number, opening: boolean, optional: boolean): string => {
    ...
    const anchorClause = opening
      ? '首个镜头必须说明与上单元锚定帧的匹配关系；'
      : '';
    return `${header}镜头${idx}：【镜头名称+运镜方式+构图方式】【时长】秒，【景别】，【光线】。双合规性声明：①机位合规性声明：本镜头使用XX机位，人物视线落点为XX，不存在直面镜头风险，符合机位规则；②对话交互合规性声明：本镜头中XX与XX保持面对面相向交流，视线精准匹配，不存在对空气说话问题，符合对话规则。人物与场景比例合规性声明：本镜头内人物与场景、道具之间的相对比例符合现实标准，与基准映射库一致，无比例失真。运镜轨迹：完整描述运镜，核心跟随主体必须带映射；如无运镜则删除本句。镜头画面完整描述：严格依据输入文案书写，所有人物、场景、物品每一次出现都必须单独标注映射；描述人物动作过渡、微动作、物理反馈、与台词同步的表情动作，以及清晰的视线落点与对话朝向；${anchorClause}如有台词则写\`角色标准名称 @图片X对角色标准名称 @图片X台词：『完整台词原文』\`。音画同步深度融合指令：写明台词起止时间、语速、声线、情绪，要求先语音、后口型/动作/运镜，逐字同步；无台词则删除本句。动作捕捉驱动指令：写明动作与语音同步捕捉、帧级对齐，对应角色必须带映射。镜头衔接锚点：${idx === 3 ? '写明本镜头作为下一单元开场基准，' : '描述切镜锚点，并明确与下一镜头之间仅允许原生画面硬切，禁止叠化；'}no dissolves, no cross-fades, use hard cuts only。镜头位置与运动：机位与运镜说明，所属场景标准名称 @图片X，与上下单元基准一致`;
  };
  return [
    '【分镜镜头内容】',
...
function renderGridShotsSection(cells: CellSpec[], duration: number, label: string): string {
  ...
  lines.push(`5. 同一人物外观 / 服装 / 持物 / 比例 / 空间编号跨镜头**完全一致**，仅允许动作 / 视线 / 景别 / 机位变化。`);
  ...
  for (const spec of cells) {
    ...
    lines.push(`- 画面（仅写客观可见画面，禁止写心理 / 旁白 / 解说）：__基于 cell ${spec.cell} 的视觉锚点；人物姿态 + 空间锚点（含床位 / 家具编号）+ Z 轴层级 + 视线方向 + 持物 + 动作 + 微动作；所有元素带 \`@图片N\` 映射__`);
    if (isFirst) {
      lines.push(`- 与上单元结尾锚定帧的匹配关系：__人数 / 站位 / 朝向 / 视线 / 持物 / 光线零偏差__`);
    }
    lines.push(`- 台词（仅当原文明示该镜头有人物开口说话时填）：\`角色 @图片X 对 角色 @图片Y 台词：『完整原文』\`；否则写"无"`);
    lines.push(`- OS/OV（仅当原文明示心理独白 / 画外音时填）：\`【对应角色】OS：『完整原文』；播报全程对应人物嘴巴闭合\`；否则写"无"`);
    if (isLast) {
      lines.push(`- 本单元结尾锚定帧（作为下单元起点）：__人数 / 站位 / 朝向 / 视线 / 持物 / 光线 / 比例的稳定收束态__`);
    } else {
  ...
  lines.push(`【自检】输出前确认：(a) 镜头编号从 1 到 ${N} 全齐 (b) 每镜头时长之和 = ${duration} 秒 (c) 没有写任何心理 / 旁白 / 解说 / 评价句。`);
  return lines.join('\n');
}
