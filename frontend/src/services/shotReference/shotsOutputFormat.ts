...
function renderGridShotsSection(cells: CellSpec[], duration: number, label: string): string {
  ...
  for (const spec of cells) {
    ...
    lines.push(`- 画面（仅写客观可见画面，禁止写心理 / 旁白 / 解说）：__基于 cell ${spec.cell} 的视觉锚点；人物姿态 + 大致空间位置（自然描述，例如"靠窗床边"，不强制硬编号）+ 上下层级（如有）+ 视线方向 + 持物 + 动作 + 微动作；所有元素带 \`@图片N\` 映射__`);
    if (isFirst) {
