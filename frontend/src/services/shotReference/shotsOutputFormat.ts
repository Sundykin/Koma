...
function renderNormalShotsSection(duration: number): string {
  ...
  const shotTemplate = (idx: number, opening: boolean, optional: boolean): string => {
    ...
    const anchorClause = opening
      ...
      : '';
    return `${header}镜头${idx}：【镜头名称+运镜方式+构图方式】【时长】秒，【景别】，【光线】。双合规性声明：①机位合规性声明：本镜头使用XX机位，人物视线落点为XX，不存在直面镜头风险，符合机位规则；②对话交互合规性声明：本镜头中XX与XX保持面对面相向交流，视线精准匹配，不存在对空气说话问题，符合对话规则。人物与场景比例合规性声明：本镜头内人物与场景、道具之间的相对比例符合现实标准，与基准映射库一致，无比例失真。运镜轨迹：完整描述运镜，核心跟随主体必须带映射；如无运镜则删除本句。镜头画面完整描述：严格依据输入文案书写，所有人物、场景、物品每一次出现都必须单独标注映射；每个人物写清主动作、微动作、重心/接触点、眉眼/嘴角/肩颈/手指等表情与可见情绪外化，动作幅度克制且与台词节拍同步；明确物理反馈、视线落点与对话朝向；${anchorClause}如有台词则写\`角色标准名称 @图片X对角色标准名称 @图片X台词：『完整台词原文』\`。音画同步深度融合指令：写明台词起止时间、语速、声线、语气和可见情绪外化，要求先语音、后口型/表情/动作/运镜，逐字同步；无台词则删除本句。动作捕捉驱动指令：写明动作与语音同步捕捉、表情肌微动和帧级对齐，对应角色必须带映射。镜头衔接锚点：${idx === 3 ? '本镜头末态即上方【本单元结尾锚定帧】（仅做收束断言，不要在此重复描述末态内容），' : '描述切镜锚点，并明确与下一镜头之间仅允许原生画面硬切，禁止叠化；'}no dissolves, no cross-fades, use hard cuts only。镜头位置与运动：机位与运镜说明，所属场景标准名称 @图片X，与上下单元基准一致`;
  };
  return [
    '【分镜镜头内容】',
...
function renderGridShotsSection(cells: CellSpec[], duration: number, label: string): string {
  ...
  for (const spec of cells) {
    ...
    lines.push(`- 画面（仅写客观可见画面，禁止写心理 / 旁白 / 解说）：__基于 cell ${spec.cell} 的视觉锚点；人物姿态 + 大致空间位置（自然描述，例如"靠窗床边"，不强制硬编号）+ 上下层级（如有）+ 视线方向 + 持物 + 主动作 + 微动作 + 表情变化 / 可见情绪外化（眉眼、嘴角、肩颈、呼吸、手指、重心）；所有元素带 \`@图片N\` 映射__`);
    if (isFirst) {
  ...
  lines.push(`【自检】输出前确认：(a) 镜头编号从 1 到 ${N} 全齐 (b) 每镜头时长之和 = ${duration} 秒 (c) 人物镜头含主动作 + 微动作 + 表情 / 可见情绪外化 (d) 没有写任何心理 / 旁白 / 解说 / 评价句。`);
  return lines.join('\n');
}
