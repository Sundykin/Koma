...
describe('VideoNode', () => {
  ...
  it('在节点内联渲染视频并隔离播放按钮事件', async () => {
    ...
    const playButton = await screen.findByRole('button', { name: '播放视频' });
  ...
  it('将本地生成视频解析成节点内可播放的媒体地址', async () => {
    vi.spyOn(electronService.fs, 'readFileAsBase64').mockResolvedValue('aGVsbG8=');
    ...
    await waitFor(() => {
      const video = document.querySelector('video');
      expect(video?.getAttribute('src')).toMatch(/^(data:video\/mp4;base64,aGVsbG8=|blob:)/);
    });
