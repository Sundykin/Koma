...
export function getFileExtensionFromPath(path: string, fallback: string): string {
  const normalized = path.split('?')[0].split('#')[0];
  const match = normalized.match(/\.([a-zA-Z0-9]{1,8})$/);
  return match?.[1]?.toLowerCase() || fallback;
}
export function getFileExtensionFromMimeType(mimeType: string | undefined, fallback: string): string {
...
export function getDefaultExtension(kind: LinghuiMediaItem['kind']): string {
  switch (kind) {
    case 'video':
      return 'mp4';
    case 'audio':
...
export function inferExtension(source: string | undefined, mimeType: string | undefined, fallback: string): string {
  if (source) {
    if (isDataUri(source)) {
...
export function resolveNodeLabel(node: LinghuiRFNodeSnapshot, fallbackIndex: number): string {
  return sanitizeSegment(String(node.data.label || `节点-${fallbackIndex + 1}`), `node-${fallbackIndex + 1}`);
}
export function resolveNodeTextValue(nodeData: LinghuiNodeData, runState?: LinghuiNodeRunState): string {
...
export function resolvePrimaryMedia(nodeData: LinghuiNodeData, runState?: LinghuiNodeRunState): LinghuiMediaItem | undefined {
...
export function dedupeMediaItems(items: LinghuiMediaItem[]): LinghuiMediaItem[] {
...
export async function exportMediaFile(params: {
  nodeDir: string;
  nodeDirName: string;
  baseName: string;
...
export async function exportJsonFile(params: {
  nodeDir: string;
  nodeDirName: string;
  fileName: string;
  content: unknown;
...
export async function exportStoryboardFrames(params: {
  nodeDir: string;
  nodeDirName: string;
...
export async function exportNodeTarget(
