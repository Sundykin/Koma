...
interface LinghuiCanvasHistorySnapshot extends LinghuiCanvasDocumentSnapshot {
  nodeRuns: Record<string, LinghuiNodeRunState>;
}
...
export function useLinghuiCanvasHistory({
  ...
  const historyPastRef = useRef<LinghuiCanvasHistorySnapshot[]>([]);
  const historyPresentRef = useRef<LinghuiCanvasHistorySnapshot | null>(null);
  const historyFutureRef = useRef<LinghuiCanvasHistorySnapshot[]>([]);
  const onGraphChangeRef = useRef(onGraphChange);
  const onRestoreNodeRunsRef = useRef(onRestoreNodeRuns);
  ...
  useEffect(() => {
    onRestoreNodeRunsRef.current = onRestoreNodeRuns;
  }, [onRestoreNodeRuns]);
  ...
  useEffect(() => {
    ...
    const liveSnapshot = buildCanvasDocumentSnapshotFromRF(
    ...
    const changeKind = detectCanvasMutationKind(historyPresentRef.current, liveSnapshot);
    if (changeKind === 'content' || changeKind === 'layout') {
      return;
    }

    historyPresentRef.current = {
      ...historyPresentRef.current,
      nodeRuns: cloneSnapshotValue(nodeRuns),
    };
  }, [nodeRuns]);
  ...
  const captureCanvasDocumentSnapshot = useCallback((): LinghuiCanvasHistorySnapshot => {
    const rfNodes = reactFlow.getNodes();
    const rfEdges = reactFlow.getEdges();
    const currentViewport = reactFlow.getViewport();
    return {
      ...buildCanvasDocumentSnapshotFromRF(rfNodes, rfEdges, {
        x: currentViewport.x,
        y: currentViewport.y,
        zoom: currentViewport.zoom,
      }),
      nodeRuns: cloneSnapshotValue(nodeRunsRef.current),
    };
  }, [reactFlow]);

  const applyCanvasDocumentSnapshot = useCallback((snapshot: LinghuiCanvasHistorySnapshot) => {
    applyingHistoryRef.current = true;
    hydratingRef.current = true;
    setNodes(buildRFNodesFromSnapshot(snapshot));
    setEdges(buildRFEdgesFromSnapshot(snapshot));
    resetUiState();
    onRestoreNodeRunsRef.current?.(cloneSnapshotValue(snapshot.nodeRuns));
  ...
  const commitHistorySnapshot = useCallback((snapshot: LinghuiCanvasHistorySnapshot) => {
  ...
  const syncHistoryBaseline = useCallback((snapshot?: LinghuiCanvasHistorySnapshot) => {
  ...
  useEffect(() => {
    ...
    resetUiState();
    const workspaceSnapshot: LinghuiCanvasHistorySnapshot = {
      graphData: cloneSnapshotValue(workspace.graphData),
      viewport: cloneSnapshotValue(workspace.viewport),
      nodeRuns: cloneSnapshotValue(workspace.nodeRuns),
    };
