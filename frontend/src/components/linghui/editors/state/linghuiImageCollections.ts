...
export async function createLinghuiImageAssetItemFromSource(params: {
  ...
  if (!source) {
    throw new Error('图片来源不能为空');
  }

  const previewSource = toFileSystemDisplayUrl(source) || source;
  let width: number | undefined;
  let height: number | undefined;
  let aspectRatio: string | undefined;

  if (previewSource && typeof Image !== 'undefined') {
    await new Promise<void>(resolve => {
      const image = new Image();
      image.onload = () => {
        width = image.naturalWidth || undefined;
        height = image.naturalHeight || undefined;
        aspectRatio = resolveImageAspectRatioLabel(width, height);
        resolve();
      };
  ...
  return {
    id: String(params.id ?? '').trim() || nanoid(10),
    source,
