...
export class OpenAICompatibleTTIProvider implements TTIProvider {
   ...
   * 支持多张参考图通过 image[] 字段。new-api 的 openai adaptor 强制要求 multipart
   * （`relay/channel/openai/adaptor.go::ConvertImageRequest` 对 RelayModeImagesEdits 的分支），
   * 无法用 JSON body 替代。
   */
  ...
  async start(request: TTIRequest): Promise<ProviderStartResult<ImageResult>> {
    ...
    // + 非 multi-angle 场景时切到 edits multipart。
    const refs = request.references || [];
    const openaiFamily = isOpenAIImageModel(this.getModelName());
    const useImageEdits = !isMultiAngle && openaiFamily !== null && refs.length > 0;
