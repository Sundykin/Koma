...
describe('getModelMaxReferenceImages', () => {
  it('优先用 model.defaults.maxReferenceImages 覆盖', () => {
    expect(getModelMaxReferenceImages(model({ maxReferenceImages: 9 }), 'comfyui-itv')).toBe(9);
  });

  it('忽略非法 maxReferenceImages 值，回退到 providerType 默认', () => {
    expect(getModelMaxReferenceImages(model({ maxReferenceImages: 0 }), 'comfyui-itv')).toBe(9);
  ...
  it('按 providerType 命中默认表', () => {
    expect(getModelMaxReferenceImages(model(), 'comfyui-itv')).toBe(9);
  ...
  it('model 为 null/undefined 时只看 providerType', () => {
    expect(getModelMaxReferenceImages(null, 'comfyui-itv')).toBe(9);
